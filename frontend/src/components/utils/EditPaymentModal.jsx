...
function EditPaymentModal({ api, onClose, onSave, paymentToEdit, errorMessage, onClearBackendError }) {
    ...
    useEffect(() => {
        if (paymentToEdit) {
            setFormData({
                id: paymentToEdit.id,
                type: paymentToEdit.type || 'espèces',
                customer_id: paymentToEdit.customer_id || '',
                contrat_id: paymentToEdit.contrat_id || null,
                description: paymentToEdit.description || '',
                amount: paymentToEdit.amount || '',
                date_payment: paymentToEdit.date_payment ? new Date(paymentToEdit.date_payment).toISOString().slice(0, 10) : '',
                category: paymentToEdit.category || 'travaux',
            });
        }
    }, [paymentToEdit]);
    ...
    return (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-lg w-full h-[90vh] flex flex-col overflow-hidden">
                <h3 className="text-lg font-semibold text-white bg-emerald-600 p-4 shadow-md">Modifier le Paiement #{formData.id}</h3>
                <div className="flex-grow overflow-y-auto p-6">
                    <form id="editPaymentForm" onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="flex flex-col p-4 border-t dark:border-gray-700">
                    ...
                    <div className="flex justify-end space-x-3">
                        ...
                        <button type="submit" form="editPaymentForm" className="px-4 py-2 bg-emerald-600 text-white rounded-md hover:bg-emerald-700 dark:bg-emerald-700 dark:hover:bg-emerald-800">Mettre à jour</button>
                    </div>
                </div>
            </div>
        </div>
