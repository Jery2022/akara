// frontend/src/components/utils/EditPaymentModal.jsx

import React, { useState } from 'react';
//import {  X } from 'lucide-react';

function EditPaymentModal ({ onClose, onSave, paymentToEdit, customers, employees }) {
    const [formData, setFormData] = useState({
        id: paymentToEdit.id,
        type: paymentToEdit.type,
        customer_id: paymentToEdit.customer_id,
        user_id: paymentToEdit.user_id,
        contrat_id: paymentToEdit.contrat_id,
        description: paymentToEdit.description,
        amount: paymentToEdit.amount,
        date_payment: paymentToEdit.date_payment,
    ...
    return (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex justify-center items-center">
            <div className="bg-white p-6 rounded-lg shadow-xl w-full max-w-md">
                <h3 className="text-xl font-bold mb-4">Modifier le Paiement #{formData.id}</h3>
                ...
                <div className="mt-6 flex justify-end space-x-2">
                    ...
                    <button onClick={handleSave} className="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 transition-colors">Mettre à jour</button>
                </div>
            </div>
        </div>
    );
