...
function CreateDepenseModal({ api, onClose, onSave, loading, errorMessage, onClearBackendError }) {
  ...
  return createPortal(
    <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-lg w-full h-[90vh] flex flex-col overflow-hidden">
        <h2 className="text-lg font-semibold text-white bg-emerald-600 p-4 shadow-md">Ajouter une Dépense</h2>
        <div className="flex-grow overflow-y-auto p-6">
          <form id="depenseForm" onSubmit={handleSubmit} className="space-y-4" noValidate>
        ...
        <div className="flex flex-col p-4 border-t dark:border-gray-700">
          ...
          <div className="flex justify-end space-x-3">
            ...
            <button type="submit" form="depenseForm" className="px-4 py-2 bg-emerald-600 text-white rounded-md hover:bg-emerald-700 dark:bg-emerald-700 dark:hover:bg-emerald-800" disabled={loading}>Ajouter</button>
          </div>
        </div>
      </div>
    </div>,
