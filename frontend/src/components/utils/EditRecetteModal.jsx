// frontend/src/components/utils/EditRecetteModal.jsx
// --- Composant Modale: Modification d'une recette ---

import React, { useState, useEffect } from 'react';
import { createPortal } from 'react-dom';

/**
 * Composant de modale pour la modification d'une recette existante.
 * @param {object} props Les propriétés du composant.
 * @param {object} props.recetteToEdit L'objet recette à modifier.
...
function EditRecetteModal({
  ...
  // Initialisation de l'état du formulaire avec les données de la recette à modifier
  const [formState, setFormState] = useState({
    ...recetteToEdit,
    date_recette: recetteToEdit?.date_recette ? recetteToEdit.date_recette.split(' ')[0] : '',
  });

  // useEffect pour calculer le total chaque fois que la quantité ou le prix changent
  useEffect(() => {
  ...
  // Rendu de la modale en dehors du flux normal du DOM,
  // ce qui garantit qu'elle s'affiche correctement au-dessus du reste du contenu.
  return createPortal(
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div className="bg-white p-6 rounded-xl shadow-2xl max-w-lg w-full">
        <h2 className="text-2xl font-bold mb-4 text-gray-800">Modifier la Recette</h2>
