...
return [
    ...
    'POST'   => function () {
        ...
        if (! isset($data['type'], $data['amount'], $data['date_vente'], $data['category'])) {
        ...
        $stmt        = $pdo->prepare("INSERT INTO ventes (type, amount, date_vente, category, user_id, customer_id, contrat_id, description) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
        if (! $stmt) {
    ...
    'PUT'    => function () {
        ...
        $data = json_decode(file_get_contents('php://input'), true);
        if (! isset($data['id'], $data['type'], $data['amount'], $data['date_vente'], $data['category'])) {
            echo json_encode(['error' => 'Champs obligatoires manquants'], JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT);
            exit;
        }
        $user_id     = $data['user_id'] ?? null;
        ...
        $contrat_id  = $data['contrat_id'] ?? null;
        $description = $data['description'] ?? null;
        $stmt        = $pdo->prepare("UPDATE ventes SET type=?, amount=?, date_vente=?, category=?, user_id=?, customer_id=?, contrat_id=?, description=? WHERE id=?");
        if (! $stmt) {
