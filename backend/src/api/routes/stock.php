...
return [
    ...
    'POST'   => function () {
        ...
        $sql         = "INSERT INTO stock (produit_id, quantity, unit, min, supplier_id, entrepot_id) VALUES (?, ?, ?, ?, ?, ?)";
    ...
    'PUT'    => function () {
        ...
        $supplier_id = $data['supplier_id'] ?? null;
        $sql         = "UPDATE stock SET produit_id=?, quantity=?, unit=?, min=?, supplier_id=?, entrepot_id=? WHERE id=?";
        $stmt        = $pdo->prepare($sql);
        $stmt->execute([
            $data['produit_id'],
            $data['quantity'],
