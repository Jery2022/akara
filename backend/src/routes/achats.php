...
switch ($_SERVER['REQUEST_METHOD']) {
    ...
    case 'POST':
        ...
        if (! isset($data['type'], $data['amount'], $data['date'], $data['category'])) {
        ...
        $stmt        = $pdo->prepare("INSERT INTO payments (type, amount, date_achat, category, user_id, supplier_id,  contrat_id, description  ) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
        if (! $stmt) {
    ...
    case 'PUT':
        // Modifie un paiement
        $data = json_decode(file_get_contents('php://input'), true);
        if (! isset($data['id'], $data['type'], $data['amount'], $data['date_acahat'], $data['category'], )) {
            echo json_encode(['error' => 'Champs obligatoires manquants']);
            exit;
        }
        $user_id     = $data['user_id'] ?? null;
        ...
        $contrat_id  = $data['contrat_id'] ?? null;
        $description = $data['description'] ?? null;
        $stmt        = $pdo->prepare("UPDATE achat SET type=?,
            amount=?,
            date_achat=?,
            category=?,
            user_id=?,
    ...
    case 'DELETE':
        ...
        $stmt = $pdo->query("DELETE FROM achats WHERE id = $id");
        if (! $stmt) {
            echo json_encode(['error' => 'Erreur lors de la préparation']);
            exit;
        }
